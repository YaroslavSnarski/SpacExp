...
from django.shortcuts import render, redirect
from django.conf import settings
from .file_manager import FileManager  # Убедитесь, что импортируете правильно
...
def select_folder(request):
    if request.method == 'POST':
        ...
        if not directory:
            # Если путь не был передан, значит ошибка выбора метода
            return render(request, 'select_folder.html', {'error': 'Пожалуйста, выберите папку для анализа.'})

        # Убедитесь, что путь передан и обработан корректно
        ...
        # Обновление базы данных через метод экземпляра
        manager.import_csv_to_db(output_csv)

        return redirect('analytics')  # Перенаправление на аналитику
